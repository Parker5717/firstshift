...
Сервер → Клиент:
  { "type": "detections", "markers": [...], "objects": [...], "ppe": {...}, "processing_ms": 45 }
  { "type": "pong" }
  { "type": "error", "message": "..." }
"""
...
async def vision_websocket(websocket: WebSocket, token: str = ""):
    ...
    Клиент отправляет кадры, сервер возвращает детекции в реальном времени.
    """
    # Авторизация через query-параметр (WS не поддерживает заголовки в браузере)
